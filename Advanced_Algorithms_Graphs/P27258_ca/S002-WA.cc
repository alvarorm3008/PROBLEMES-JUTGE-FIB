...
bool bfs (int n, int m, Tablero& t, pos act) {
    ...
    for (int i = 0; i < 4; ++i) {
        act = {act.first + X[i], act.second + Y[i]};
        if (ok (n, m, act.first, act.second) and t[act.first][act.second] != 'X') {
            if (bfs(n, m, t, act)) return true;
        }
    }
    return false;
}
    ...
    while (cin >> n >> m) {
        ...
        for (int i = 0; i < n; ++i) {
            for (int j = 0; j < m; ++j) {
                ...
                if (t[i][j] == 'M') monstres.push_back ({i, j});
            }
        }
