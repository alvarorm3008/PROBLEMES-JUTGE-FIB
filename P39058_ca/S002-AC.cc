...
bool pos_valida (int n, int m, int x, int y, Tablero& t) {
    return x >= 0 and y >= 0 and x < n and y < m and t[x][y] != 'a';
}

void bfs (Tablero& t, int f, int c, int& flors, int& distancia) {
    int n = t.size();
    int m = t[0].size();
    queue<pos> q;
        ...
        for (int i = 0; i < 8; ++i) {
            ...
            if (pos_valida(n, m, x_seg, y_seg, t)) {
                if (dist[x_seg][y_seg] == -1) {
                    dist[x_seg][y_seg] = dist_act+1;
                    if (t[x_seg][y_seg] == 'F') {
                        ++flors;
                        distancia += dist[x_seg][y_seg];
                    }
                    q.push ({x_seg, y_seg});
                }
            }
